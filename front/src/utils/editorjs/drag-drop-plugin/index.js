...
export default class DragDrop {
   ...
   * @param focusToolbar: function
   *   focusToolbar — focus
   */
  constructor({ editor, focusToolbar }) {
    const { configuration, blocks } = editor;
    this.api = blocks;
    this.editor = editor;
    ...
    document.addEventListener('drop', (event) => {
      ...
      if (this.holder.contains(target)) {
        ...
        if (dropTarget) {
          ...
          this.moveBlocks();
          const forceBlock = this.api.getBlockByIndex(this.endBlock)?.holder;
          this.focusToolbar({ forceBlock });
        }
      }
    });
  }
